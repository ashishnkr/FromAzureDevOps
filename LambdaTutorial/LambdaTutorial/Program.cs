...
namespace LambdaTutorial
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var subList = listOne.Union(listTwo);
            List<int> newSubList = subList.ToList();
            List<int> newSubList2 = newSubList;
            newSubList.RemoveAt(1);
            newSubList2.RemoveAt(1);

            /*Changes done in newSubList2 is also getting reflected in newSubList as
             * object reference is being assigned in the previous steps and we are not creating a new list */

            subList.ToList().ForEach(x => Console.WriteLine("Printing List: {0}", x));
            newSubList.ForEach(x => Console.WriteLine("Printing List after removal {0}", x));
            newSubList2.ForEach(x => Console.WriteLine("Printing List after removal from newSubList2: {0}", x));
